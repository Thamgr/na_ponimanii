...
Если это уместно, в последнем абзаце дай конкретные примеры
"""
...
# Bot Messages
# Start command
BOT_WELCOME_MESSAGE = 'Привет, я твой бот! Используй /add чтобы добавить новую тему для изучения. Используй /list чтобы увидеть свои темы. Используй /topic чтобы получить объяснение случайной темы.'

# Add topic
BOT_EMPTY_TOPIC_ERROR = 'Нужно указать тему'
BOT_TOPIC_ADDED_SUCCESS = "Тема сохранена: {title}\n\nЯ подготовлю объяснение этой темы. Скоро вы сможете его увидеть, используя команду /topic"
...
# Get topic command
BOT_NO_TOPICS_FOR_EXPLANATION = 'У вас нет сохраненных тем. Используйте /add чтобы добавить тему для изучения.'
